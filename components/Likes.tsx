...
const Likes = ({ post_id, user_id, upVotes }: { user_id: string; post_id: string; upVotes: string[] }) => {
  ...
  return (
    <Pressable
      className={`flex-row items-center justify-center py-2 ${isUpvoted ? 'opacity-100' : 'opacity-80'}`}
      onPress={handlePress}
      style={{ flex: 1 }}
    >
      <Ionicons 
      ...
      />
      <Text 
        className={`ml-1 text-sm ${isUpvoted ? 'text-[#0a66c2] font-medium' : 'text-gray-500'}`}
      >
        Like{isUpvoted ? 'd' : ''}
      </Text>
    </Pressable>
