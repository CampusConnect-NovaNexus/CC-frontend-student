...
const GrievanceScreen = () => {
  ...
  useEffect(() => {
    const fetchGrievances = async () => {
      try {
        const response = await fetch(`${BASEURL}/api/grievance`);
        const data = await response.json();
        setGrievances(data);
      } catch (err) {
        console.error(err);
        Toast.show({ type: 'error', text1: 'Error fetching grievances' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async () => {
    if (!grievanceText.trim()) {
      Toast.show({ type: 'error', text1: 'Grievance cannot be empty' });
      return;
    }

    try {
      const res = await fetch(`${BASEURL}/api/grievance/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ grievance: grievanceText, username }),
      });

      const newGrievance = await res.json();
      setGrievances([newGrievance, ...grievances]);
      setGrievanceText('');
      Toast.show({ type: 'success', text1: 'Grievance submitted' });
    } catch (error) {
      console.error(error);
      Toast.show({ type: 'error', text1: 'Failed to submit grievance' });
    }
  };
    ...
    <SafeAreaView style={tw`flex-1 bg-white`}>
      ...
      >
        <View style={tw`flex-1 p-4`}>
          <Text style={tw`text-2xl font-bold text-vsyellow mb-4`}>Raise a Grievance</Text>
          <View style={tw`bg-gray-100 rounded-xl p-3 mb-4`}>
            <TextInput
              placeholder="What's troubling you?"
              value={grievanceText}
              onChangeText={setGrievanceText}
              style={tw`h-20 text-base`}
              multiline
            />
            <TouchableOpacity
              onPress={handleSubmit}
              style={tw`absolute bottom-2 right-2 bg-vsyellow p-2 rounded-full`}
            >
              <Ionicons name="send" size={20} color="black" />
            </TouchableOpacity>
          </View>

          <Text style={tw`text-xl font-semibold mb-3`}>Your Grievances</Text>
          {loading ? (
